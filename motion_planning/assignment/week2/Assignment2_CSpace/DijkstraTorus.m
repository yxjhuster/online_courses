...
while true
    ...
    if i + 1 <= size(map,1)
        update ( i + 1, j,next_distance,current);
    else
        update ( 1, j,next_distance,current);
    end
    if i - 1 > 0
        update ( i - 1, j,next_distance,current);
    else
        update ( size(map,1), j,next_distance,current);
    end
    if j - 1 > 0
        update (i, j - 1,next_distance,current);
    else
        update (i, size(map,2),next_distance,current);
    end
    if j + 1 <= size(map, 2)
        update (i, j + 1,next_distance,current);  
    else
        update (i, 1,next_distance,current); 
    end
